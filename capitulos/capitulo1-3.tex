...
\begin{proof}
  ...
  \[ T^{-1}_{B',B} T_{B,B'} = (T^{-1}\circ T)_{B,B} = (1_{L(V,W)})_{B,B} = I_n, \]
  ...
  Ahora, si $T_{B,B'}$ es invertible entonces $m = n$ y por el teorema \ref{teo:isomTrMat} existe una transformación lineal $S\colon W \to V$ tal que $S_{B,B'} =  (T_{B,B'})^{-1}$. Aplicando el teorema anterior, esto implica que
    \[ (S \circ T)_{B,B'} =  S_{B,B'}T_{B,B'} = I_n = (1_{L(V,W)})_{B,B'}. \]
  De nuevo, por el teorema \ref{teo:isomTrMat}, tenemos que $S \circ T = 1_{L(V,W)}$ y análogamente se tiene que $T \circ S = 1_{L(V,W)}$, mostrando así que $T$ es invertible.
\end{proof}



\section{Semejanza de matrices y transformaciones lineales}
